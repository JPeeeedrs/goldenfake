...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Expand Wikipedia categories with their subcategories")
    parser.add_argument("input_json", type=Path, help="Path to the JSON file describing category themes")
    parser.add_argument(
        "--output",
        type=Path,
...
def load_category_map(path: Path) -> Dict[str, List[str]]:
    ...
    if not isinstance(payload, Mapping):
        raise TypeError("Input JSON must be an object mapping theme names to category lists")
    ...
    for theme, values in payload.items():
        if isinstance(values, str):
            ...
            items = [str(item) for item in values if isinstance(item, (str, bytes))]
        else:
            raise TypeError(f"Expected list of strings for theme '{theme}', got {type(values)!r}")
    ...
    while True:
        ...
        while True:
            ...
            try:
                ...
                if attempt >= retries:
                    raise RuntimeError(f"Failed to fetch subcategories for {title}: {exc}") from exc
                logging.warning(
        ...
        cmcontinue = cont.get("cmcontinue") if isinstance(cont, Mapping) else None
        if not cmcontinue:
            break
...
def expand_categories(
    ...
    for theme, categories in input_map.items():
        ...
        for category in categories:
            ...
            try:
                fetched = fetch_subcategories(session, category, delay, endpoint)
            except RuntimeError as exc:
                logging.error("Falha ao buscar subcategorias de %s: %s", normalized, exc)
                continue
...
def main() -> None:
    ...
    with requests.Session() as session:
        ...
        expanded = expand_categories(session, input_map, args.delay, args.api_endpoint)
