...
document.addEventListener("DOMContentLoaded", () => {
	...
	el("run").onclick = async () => {
		...
		const body = {
			text,
			k: Number(el("k")?.value || 20),
			w_hist: Number(el("w_hist")?.value || 0.5),
			w_bert: Number(el("w_bert")?.value || 0.35),
			w_fontes: Number(el("w_fontes")?.value || 0.33),
			max_tokens: Number(el("max_tokens")?.value || 512),
			overlap_tokens: Number(el("overlap_tokens")?.value || 128),
			hist_agg: el("hist_agg")?.value || "max",
			bert_agg: el("bert_agg")?.value || "mean",
			use_wiki: el("use_wiki")?.value === "true",
			wiki_titles: Number(el("wiki_titles")?.value || 3),
			w_faiss: Number(el("w_faiss")?.value || 0.5),
			w_wiki: Number(el("w_wiki")?.value || 0.5),
		};
