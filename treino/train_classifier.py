...
def load_dataset(path):
    ...
    data = [x for x in data if isinstance(x, dict) and isinstance(
        x.get("text"), str) and x.get("text").strip() and x.get("label")]
...
def embed_texts(model, texts: List[str], batch_size=64, normalize=True,
                max_tokens: int = 512, overlap_tokens: int = 128):
    # Função para dividir texto em chunks de max_tokens tokens
    ...
    for text in texts:
        ...
        chunk_embs = model.encode(chunks, batch_size=batch_size, convert_to_numpy=True,
...
PROPER_SEQ_RE = re.compile(
    rf"\b([{_CAP}][{_LOW}]+(?:\s+[{_CAP}][{_LOW}]+){{0,3}})\b")
...
def compute_protected_sample_weights(texts: List[str], labels: np.ndarray, terms: List[str], mode: str, nlp, ent_types: Iterable[str]) -> np.ndarray:
    ...
    """
    n = len(texts)
    in_sub = np.array([contains_protected_dynamic(
        t, terms, mode, nlp, ent_types) for t in texts], dtype=bool)
...
def main():
    parser = argparse.ArgumentParser(
        description="Treino classificador com opções de debias (auto nomes próprios/NER) e estilo")
    parser.add_argument(
        "--debias", choices=["none", "mask", "weight", "both"], default="both", help="Estratégia de debias")
    parser.add_argument("--protected_mode", choices=["list", "auto", "both"], default="auto",
                        help="Fonte de menções protegidas: lista, auto (NER/regex) ou ambos")
    parser.add_argument("--entity_types", type=str, default="PERSON,ORG,GPE",
                        help="Tipos de entidades (spaCy) para mascarar, separados por vírgula")
    parser.add_argument("--protected_terms_file", type=str, default=os.path.join(
        BASE_DIR, "debias_config.json"), help="Arquivo JSON com protected_terms")
    parser.add_argument("--augment_alpha", type=float, default=0.5,
                        help="Peso das cópias mascaradas (se mask/both)")
    parser.add_argument("--test_size", type=float, default=0.2)
    parser.add_argument("--random_state", type=int, default=42)
    parser.add_argument("--use_style", action="store_true",
                        help="Adicionar features de estilo ao classificador")
    parser.add_argument("--style_scale", type=float, default=0.2,
                        help="Fator para reduzir a influência das features de estilo após padronização")
    parser.add_argument("--calibrate", action="store_true",
                        help="Aplicar CalibratedClassifierCV (isotonic, cv=5) para calibrar probabilidades")
    ...
    X_train_txt, X_test_txt, y_train, y_test = train_test_split(
        texts, y_all, test_size=args.test_size, random_state=args.random_state, stratify=y_all)

    print("Carregando modelo de embeddings...")
    sbert = SentenceTransformer(EMBED_MODEL_NAME)
    # Ajustar janela padrão para 512
    try:
        sbert.max_seq_length = 512
        if hasattr(sbert, "tokenizer") and sbert.tokenizer is not None:
            sbert.tokenizer.model_max_length = 512
    except Exception:
        pass
    ...
    if args.debias in ("mask", "both"):
        ...
        if len(aug_weights) > len(X_train_txt):
            sample_weight = np.concatenate(
                [sample_weight, aug_weights[len(X_train_txt):]])

    if args.debias in ("weight", "both"):
        w = compute_protected_sample_weights(
            texts_train_for_embed, y_train_for_fit, protected_terms, args.protected_mode, _NLP, ent_types)
        if w.shape[0] != len(texts_train_for_embed):
            w = np.ones(len(texts_train_for_embed), dtype=float)
        sample_weight = w

    print("Gerando embeddings de treino (chunks 512/overlap 128)...")
    X_train_sbert = embed_texts(
        sbert, texts_train_for_embed, max_tokens=512, overlap_tokens=128)

    if args.use_style:
        X_train_style = np.vstack([extract_style_features(t)
                                  for t in texts_train_for_embed])
        style_scaler = StandardScaler()
        X_train_style_s = style_scaler.fit_transform(
            X_train_style) * float(args.style_scale)
    ...
    X_test_sbert = embed_texts(
        sbert, X_test_txt, max_tokens=512, overlap_tokens=128)
    if args.use_style:
        X_test_style = np.vstack([extract_style_features(t)
                                 for t in X_test_txt])
        X_test_style_s = style_scaler.transform(
            X_test_style) * float(args.style_scale)
    ...
    # Salvar textos de treino para uso no FAISS (evitar data leaking)
    ...
    print("Textos de treino salvos em models/train_texts.json para indexação FAISS sem data leaking.")
