...
def process_embeddings_optimized(
    ...
    for batch_idx, batch in enumerate(tqdm(dataloader, desc="Processing articles")):
        ...
        if stats["processed"] > 0 and stats["processed"] % checkpoint_interval == 0:
            ...
            if storage_mode == "compressed":
                _save_compressed_batch(output_dir, all_embeddings, metadata, stats["processed"])
    ...
    if storage_mode == "compressed":
        _save_compressed_batch(output_dir, all_embeddings, metadata, stats["processed"], final=True)
...
def _save_compressed_batch(output_dir: Path, embeddings_dict: dict, metadata: dict, batch_id: int, final: bool = False):
    ...
    logging.info(f"  Compressed: {compressed_size / 1024**2:.1f} MB ({ratio:.1f}%)")
...
def _merge_compressed_batches(output_dir: Path):
    ...
    for batch_file in tqdm(batch_files, desc="Loading batches"):
        ...
        meta_file = batch_file.parent / batch_file.name.replace("embeddings_", "metadata_").replace(".npz", ".json")
        if meta_file.exists():
    ...
    logging.info(f"  Compressed: {compressed / 1024**3:.2f} GB ({compressed/uncompressed*100:.1f}%)")
...
def _save_checkpoint(path: Path, stats: dict, finished: bool = False):
    ...
    logging.info(f"Checkpoint: {stats['processed']} articles, {stats['total_chunks']} chunks")


def main():
    parser = argparse.ArgumentParser(description="Optimized Wikipedia embedding generator")
    parser.add_argument("articles_path", type=Path, help="Path to articles JSON/JSONL")
    ...
    # Process embeddings
    logging.info(f"Starting embedding generation (storage: {args.storage_mode})...")
    ...
    logging.info(f"✓ Written {stats['written']:,} embeddings to {args.output_dir}")
    
    if args.storage_mode == "compressed":
        final_file = args.output_dir / "embeddings_all.npz"
        if final_file.exists():
            size_gb = final_file.stat().st_size / 1024**3
            logging.info(f"✓ Final compressed file: {final_file} ({size_gb:.2f} GB)")
